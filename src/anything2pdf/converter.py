#!/usr/bin/env python3
"""
anything2pdf - A feature-rich terminal tool to convert images to PDF

Usage:
    anything2pdf photo.jpg
    anything2pdf *.jpg -o album.pdf -s a4
    anything2pdf images/ --grid 2x2 --page-size letter
    anything2pdf *.png --info
"""
...
class PDFBuilder:
    ...
    def _set_metadata(self, c: 'canvas.Canvas'):
        ...
        c.setCreator(f"anything2pdf v{VERSION}")
...
def scan_script_directory() -> List[Dict[str, Any]]:
    """
    Scan all immediate subdirectories of the current working directory and
...
def display_folder_table(folders: List[Dict[str, Any]]) -> None:
    ...
    if HAS_RICH:
        tbl = Table(
            title="[bold]Folders in Current Directory[/bold]",
            show_header=True,
            header_style="bold cyan",
            border_style="dim",
        )
...
def select_folder(folders: List[Dict[str, Any]]) -> Path:
    ...
    if not folders_with_images:
        print_warning("No subfolders with images found. Using current directory.")
        return script_dir
    ...
    if HAS_INQUIRER:
        try:
            ...
            choices.append(Choice(value=script_dir, name="[Current directory]"))
    ...
    try:
        ...
        pass
    print_warning("Invalid selection, using current directory.")
    return script_dir
...
def interactive_mode(args: argparse.Namespace) -> List[Path]:
    ...
    if not args.quiet:
        print_info("Scanning current directory for folders...")
    ...
    if folders:
        ...
        selected_folder = Path.cwd()
        if not args.quiet:
            print_info("No subfolders found. Using current directory.")
...
def main():
    ...
    if not args.quiet and HAS_RICH:
        console.print(Panel.fit(
            f"[bold cyan]anything2pdf[/bold cyan] [dim]v{VERSION}[/dim]  -  Image to PDF Converter",
            border_style="cyan",
            padding=(0, 2),
        ))
